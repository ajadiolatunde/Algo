...
public class Boards {
        ...
        for (int row=0;row<board.length;row++ ){
            for (int col=0;col<board[0].length;col++ ){
                ...
                if (current== board[row][col]) {
                        ...
                        //for down
                        if (row+1<row_len && !word_coordList.contains((row+1)+""+col)  &&  word.length()>1 &&  word.charAt(1) == board[row+1][col] ){
                            count_tjunctions++;
                            down = true;
                        }
                        //for up
                        if (row>0 && !word_coordList.contains((row-1)+""+col) &&  word.length()>1 &&  word.charAt(1) == board[row-1][col] ){
                            up = true;
                            count_tjunctions++;
                        }
                        //Forward or column
                        if (col+1<col_len && !word_coordList.contains(row+""+(col+1)) &&  word.length()>1 &&  word.charAt(1) == board[row][col+1] ){
                            forward = true;
                            count_tjunctions++;
                        }
                        //backward
                        if (col>0 && !word_coordList.contains(row+""+(col-1)) &&  word.length()>1 &&  word.charAt(1) == board[row][col-1] ){
                            back = true;
                            count_tjunctions++;
                        }
                        ...
                        }else if(String.valueOf(current).equals(word) ){
                            //if only one letter word
                            return true;
                        }else{
                            // Nothing found
    ...
    public static void main(String[] args){
        ...
        String e = "SEA";
        char[][] fe = {{'A','A','A'},{'A','A','A'},{'A','A','A'}};
        String f = "AAAB";
        System.out.println(exist(fe,f));
